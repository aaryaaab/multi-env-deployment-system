    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 bg-white/80 dark:bg-slate-900/60 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800/80 space-y-6 relative z-10">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-2">Create account</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Register to start managing and tracking tasks.</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className={`w-full py-2.5 font-semibold text-white bg-indigo-600 hover:bg-indigo-750 dark:bg-indigo-500 dark:hover:bg-indigo-600 rounded-lg transition-all text-sm shadow-md shadow-indigo-500/10 active:scale-[0.98] flex items-center justify-center gap-2 mt-6 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
            {isLoading ? (
              <>
